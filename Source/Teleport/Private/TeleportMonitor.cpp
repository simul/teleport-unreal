...
void ATeleportMonitor::BeginPlay()
{
	...
	if (teleport::server::serverSettings.enableGeometryStreaming)
	{
...
void ATeleportMonitor::PostEditChangeProperty(FPropertyChangedEvent &PropertyChangedEvent)
{
	...
	{
		if (teleport::server::serverSettings.enableGeometryStreaming ==  true)
		{
			InitialiseGeometrySource();
		}
...
{
	return &teleport::server::serverSettings;
}
...
void ATeleportMonitor::UpdateServerSettings()
{
	...
	const UTeleportSettings *TeleportSettings=GetDefault<UTeleportSettings>();
	teleport::server::serverSettings = {
		RequiredLatencyMs,
...
void ATeleportMonitor::CreateSession(avs::uid clientID)
{
	...
	clientSettings.shadowmapSize = teleport::server::serverSettings.defaultShadowmapSize;
	teleport::core::ClientDynamicLighting clientDynamicLighting;
	clientSettings.captureCubeTextureSize = teleport::server::serverSettings.captureCubeSize;
	clientSettings.backgroundMode = teleport::core::BackgroundMode::COLOUR;
	clientSettings.backgroundColour = {0, 0, 1.f, 0};
	clientSettings.drawDistance = teleport::server::serverSettings.detectionSphereRadius;
	...
	int perspectiveWidth = teleport::server::serverSettings.perspectiveWidth;
	int perspectiveHeight = teleport::server::serverSettings.perspectiveHeight;
	...
	{
		if (teleport::server::serverSettings.usePerspectiveRendering)
		{
	...
	if (teleport::server::serverSettings.useAlphaLayerEncoding)
	{
	...
	uint2 webcamSize = {(unsigned)teleport::server::serverSettings.webcamWidth, (unsigned)teleport::server::serverSettings.webcamHeight};
	...
	if (clientSettings.backgroundMode == teleport::core::BackgroundMode::VIDEO)
	{
		...
		if (teleport::server::serverSettings.usePerspectiveRendering)
		{
			clientSettings.videoTextureSize.x = std::max(clientSettings.videoTextureSize.x, teleport::server::serverSettings.perspectiveWidth);
			clientSettings.videoTextureSize.y = std::max(clientSettings.videoTextureSize.y, teleport::server::serverSettings.perspectiveHeight);
		}
		else
		{
	...
	if (teleport::server::serverSettings.enableWebcamStreaming)
	{
